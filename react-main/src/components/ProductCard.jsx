...
const ProductCard = ({ product }) => {
  ...
  const handleWhatsAppClick = () => {
    ...
    if (phone) {
      ...
      const message = encodeURIComponent(`Hello, I'm interested in this product: ${product.name}`);
      window.open(`https://wa.me/${cleanPhone}?text=${message}`, '_blank');
    } else {
      alert('WhatsApp contact not available for this cooperative');
    }
  };

  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col">
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="mb-4">
          <p className="text-sm text-gray-500 mb-1">Coopérative:</p>
